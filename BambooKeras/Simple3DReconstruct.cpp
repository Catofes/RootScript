...
typedef tuple<int, double, double, double> ElectronInfo; // channel_id,t,x,y
...
void T3DConvert::raw2wave(const RawHits &input)
{
    ...
    for (auto i = 0; i < input.xd.size(); i++) {
        ...
        for (auto j = 0; j < electron_numbers; j++) {
            ...
            //int channel_id = 1;
            if (channel_id >= 0) {
                electron_info.push_back(make_tuple(channel_id, tt, xx, yy));
                record_count++;
    ...
    auto track_info = map<tuple<int, int, int>, int>();
    double center_x = 0, center_y = 0, center_z = 0;
    int count = get<2>(trigger_info) - get<0>(trigger_info);
    auto trigger_offset = get<1>(electron_info[get<1>(trigger_info)]);
    for (int i = get<0>(trigger_info); i < get<2>(trigger_info); i++) {
        center_x += get<2>(electron_info[i]);
        center_y += get<3>(electron_info[i]);
        auto t = int(floor((get<1>(electron_info[i]) - trigger_offset) / 0.2 + 256));
        auto z = t * 0.2 * drift_velocity;
        center_z += z;
    }
    center_x /= count;
    center_y /= count;
    ...
    for (int i = get<0>(trigger_info); i < get<2>(trigger_info); i++) {
        ...
        if (track_info.find(key) == track_info.end()) {
            track_info[key] = 1;
        } else {
            track_info[key]++;
        }
